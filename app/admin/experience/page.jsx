...
export default function ExperienceManagementPage() {
  ...
  return (
        ...
        {experiences.map((exp) => (
          <Card key={exp._id}>
            <CardImage src={exp.img} alt={exp.company} />
                  ...
                  <IconButton variant="delete" onClick={() => handleDelete(exp._id!)}>
                    <FiTrash2 />
                  </IconButton>
                </ActionButtons>
              </CardHeader>
              {exp.desc}</CardDescription>
              {exp.skills.length > 0 && (
      ...
      <Modal isOpen={isModalOpen}>
            ...
            {editingItem ? 'Edit' : 'Add'} Experience</ModalTitle>
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                <FiUpload />
                {isUploading ? 'Uploading...' : companyImage ? 'Change Image' : 'Upload Image'}
              </ImageUploadButton>
              {companyImage && <img src={companyImage} alt="Preview" style={{ width: '80px', height: '80px', borderRadius: '8px', marginTop: '8px' }} />}
              <input
                id="company-image"
                type="file"
                accept="image/*"
              ...
              {errors.role && {errors.role.message}</ErrorMessage>}
            </FormGroup>

            
              Company *</Label>
              <Input placeholder="Company Name" {...register('company')} />
              {errors.company && {errors.company.message}</ErrorMessage>}
            </FormGroup>
              ...
              Description *</Label>
              <Textarea placeholder="Describe your role and responsibilities..." {...register('desc')} />
              ...
              <ImageUploadButton type="button" onClick={() => setSkills([...skills, ''])}>
                + Add Skill
              </ImageUploadButton>
            </FormGroup>

            
              Document URL (Optional)</Label>
              <Input placeholder="https://example.com/certificate.pdf" {...register('doc')} />
              {errors.doc && {errors.doc.message}</ErrorMessage>}
            </FormGroup>
