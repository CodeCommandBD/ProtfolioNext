...
async function getBioData() {
  try {
    const res = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/bio`, {
...
async function getExperienceData() {
  try {
    const res = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/experience`, {
      cache: 'no-store',
    });
    if (!res.ok) return [];
    return res.json();
...
async function getEducationData() {
  try {
    const res = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/education`, {
      cache: 'no-store',
    });
    if (!res.ok) return [];
    return res.json();
